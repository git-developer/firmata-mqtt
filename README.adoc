...
device, and a I<rate> per hour is calculated as `3600 / (frequency * duration)`.
...
== Examples
. S0 energy meters emitting 1000 impulses per kWh
+
  environment:
    FIRMATA_PINS: "2,3,4"
    FIRMATA_TEMPLATE: "pullup:debounce:falling:1:W"
...
. Different devices with lower sample rate
+
  environment:
